...
#define PC_DIRECT_JUMP UINT64_MAX  // Para BR y otros saltos absolutos

// Actualiza las flags Z y N basadas en el resultado
void set_flags(int64_t result) {
    NEXT_STATE.FLAG_Z = (result == 0);
    NEXT_STATE.FLAG_N = (result < 0);
}

// Ejecuta la instrucción decodificada y devuelve el offset del PC
uint64_t execute(const Instruction* inst) {
    ...
    if (strcmp(inst->name, "ADDS_IMM") == 0) {
        ...
        offset = inst->imm;  // ya fue shift-left 2 durante decode
    }
        ...
        uint64_t addr = CURRENT_STATE.REGS[inst->Rn] + inst->imm;
        uint8_t val = mem_read_32(addr) & 0xFF;
        NEXT_STATE.REGS[inst->Rt] = val;
    }

    else if (strcmp(inst->name, "LDURH") == 0) {
        uint64_t addr = CURRENT_STATE.REGS[inst->Rn] + inst->imm;
        uint16_t val = mem_read_32(addr) & 0xFFFF;
        NEXT_STATE.REGS[inst->Rt] = val;
    }
    ...
    // Por defecto, avanzar a la siguiente instrucción
    return offset;
}
